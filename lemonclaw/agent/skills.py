...
class SkillsLoader:
    ...
    def _parse_skill_metadata(self, raw: str) -> dict:
        """Parse skill metadata JSON from frontmatter (supports nanobot and lemonclaw keys)."""
        try:
            data = json.loads(raw)
            return data.get("lemonclaw", data.get("openclaw", {})) if isinstance(data, dict) else {}
