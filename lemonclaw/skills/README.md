...
Each skill lives in its own folder and must include a `SKILL.md` file with YAML frontmatter. Skills are optional guidance sources, not mandatory execution frameworks.

## Authoring Rules

- Pick one primary pattern per skill: `tool-wrapper`, `generator`, `reviewer`, `inversion`, or `pipeline`.
- Keep `SKILL.md` focused on entry rules, guardrails, and the minimum workflow when the skill is explicitly loaded.
...
- Avoid relying on built-in skills as hidden execution requirements. Runtime capabilities and live system truth should remain usable without a skill.
...
| `memory` | Two-layer memory workflow (`MEMORY.md` + `HISTORY.md`) | Built in |
...
- Skills with unmet `requires` metadata are still marked unavailable in the WebUI, but their absence must not become a hidden hard dependency for runtime execution.
