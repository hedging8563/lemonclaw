...
def get_webui_routes(
    *,
    ...
    def _attach_trigger_context(payload: dict[str, Any]) -> dict[str, Any]:
        return attach_trigger_context(payload, trigger_runtime)

    def _build_task_bundle(task_id: str) -> dict[str, Any] | None:
        ledger = getattr(agent_loop, "ledger", None)
        if ledger is None:
            return None
        return build_task_bundle(ledger, task_id, trigger_runtime=trigger_runtime)
    ...
    async def export_task(request: Request) -> Response:
        ...
        if fmt == "json":
            ...
            return resp
        if fmt == "md":
            resp = PlainTextResponse(render_task_export_markdown(export_view, task_id), media_type="text/markdown; charset=utf-8")
            _maybe_refresh_cookie(request, resp)
            return resp
    ...
    async def export_task_bundle(request: Request) -> Response:
        ...
        if fmt == "json":
            ...
            return resp
        if fmt == "md":
            resp = PlainTextResponse(render_task_bundle_markdown(bundle, task_id), media_type="text/markdown; charset=utf-8")
            _maybe_refresh_cookie(request, resp)
            return resp
        return _json({"error": "unsupported format"}, 400)
    ...
    async def get_task_postmortem(request: Request) -> Response:
        ...
        if fmt == "json":
            ...
            return resp
        if fmt == "md":
            resp = PlainTextResponse(render_task_postmortem_markdown(postmortem, task_id), media_type="text/markdown; charset=utf-8")
            _maybe_refresh_cookie(request, resp)
            return resp
        return _json({"error": "unsupported format"}, 400)
