    ...
    <div style={shellStyle}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', marginBottom: '10px' }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ fontFamily: 'var(--font-ui)', fontSize: '15px', color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={{ fontSize: '15px', color: 'var(--text-secondary)', lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>{summaryMessage}</div>
        </div>
          ...
          <div style={{ display: 'grid', gap: '8px', maxHeight: '320px', overflowY: 'auto', paddingRight: '4px' }}>
            {plans.value.length === 0 ? (
              <div style={{ padding: '12px', background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: '6px' }}>
                <div style={{ fontSize: '15px', color: 'var(--text-muted)' }}>{t('no_plans')}</div>
              </div>
            ) : (
              plans.value.map((plan) => (
                <div key={plan.request_id} style={{ background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: '6px', padding: '12px', position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ position: 'relative', zIndex: 1 }}>
                    ...
                    <div style={{ fontSize: '15px', color: 'var(--text-primary)', lineHeight: 1.5, marginBottom: plan.subtasks?.length ? '10px' : '0' }}>{plan.message}</div>
                    {plan.subtasks?.length ? (
                      <details>
                        <summary style={{ cursor: 'pointer', fontSize: '15px', color: 'var(--text-muted)', fontFamily: 'var(--font-ui)' }}>{`subtasks · ${plan.subtasks.length}`}</summary>
                        <div style={{ display: 'grid', gap: '6px', marginTop: '8px' }}>
                          {plan.subtasks.map((subtask: any) => (
                            <div key={subtask.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', fontSize: '15px', fontFamily: 'var(--font-ui)' }}>
                              <span style={{ color: subtask.status === 'completed' ? 'var(--success)' : subtask.status === 'executing' ? 'var(--accent)' : 'var(--text-muted)', marginTop: '2px', fontSize: '15px' }}>
