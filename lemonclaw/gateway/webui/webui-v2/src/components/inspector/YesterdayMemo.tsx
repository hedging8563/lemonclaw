    ...
    <div style={shellStyle}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', marginBottom: '10px' }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ fontFamily: 'var(--font-ui)', fontSize: '13px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '8px' }}>
            // {t('memo_title')}
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontFamily: 'var(--font-ui)', fontSize: '13px', color: 'var(--text-muted)' }}>{memo?.date || '—'}</span>
      ...
      >
        {!hasAnyContent ? (
          <div style={{ fontSize: '13px', color: 'var(--text-muted)', lineHeight: 1.6 }}>{t('memo_empty')}</div>
        ) : (
          <div style={{ display: 'grid', gap: '12px' }}>
            {memo?.today?.length ? (
              <div style={{ background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: '6px', padding: '10px' }}>
                <div style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '6px', fontWeight: 'bold' }}>{t('memo_today')}</div>
            ...
            {memo?.yesterday?.length ? (
              <details open>
                ...
                <div style={{ marginTop: '8px', background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: '6px', padding: '10px', fontSize: '13px', color: 'var(--text-primary)', lineHeight: 1.55, whiteSpace: 'pre-wrap' }}>
            ...
            <div style={{ background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: '8px', padding: '10px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px', marginBottom: historySectionOpen ? '8px' : '0' }}>
                <button
                  ...
                  style={{ ...pillStyle(historySectionOpen), fontSize: '13px' }}
                >
              ...
              {historySectionOpen ? (
                <div>
                {historyEntries.length === 0 ? (
                  <div style={{ fontSize: '13px', color: 'var(--text-muted)' }}>{t('memory_empty_history')}</div>
                ) : (
                  <>
                    <div
                      ref={historyRef}
                    ...
                    >
                      <div style={{ display: 'grid', gap: '8px' }}>
                        {historyEntries.map((entry, index) => (
                          <div key={`memo-history-${index}`} style={{ border: '1px solid var(--border)', borderRadius: '6px', padding: '8px', fontSize: '13px', color: 'var(--text-primary)', lineHeight: 1.55, whiteSpace: 'pre-wrap' }}>
                            {entry}
                          </div>
                        ))}
                      </div>
