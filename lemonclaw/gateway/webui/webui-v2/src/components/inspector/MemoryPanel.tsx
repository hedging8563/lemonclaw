...
export function MemoryPanel() {
      ...
      >
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
          ...
          <button
            ...
            style={{ background: 'var(--purple)', border: 'none', borderRadius: '4px', color: '#fff', cursor: 'pointer', fontSize: '15px', padding: '4px 8px' }}
          >
    ...
    <div key={doc.doc_id} style={{ border: '1px solid var(--border)', borderRadius: '6px', padding: '8px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', marginBottom: '6px', alignItems: 'center' }}>
        ...
        >
          ...
          <div style={{ fontSize: '15px', color: 'var(--text-muted)', wordBreak: 'break-word' }}>{doc.source}</div>
        </div>
      ...
      {doc.last_hit_query ? (
        <div style={{ fontSize: '15px', color: 'var(--text-secondary)', marginTop: '6px', whiteSpace: 'pre-wrap' }}>
          {`${t('knowledge_last_query')}: ${doc.last_hit_query}`}
        </div>
      ) : null}
      {doc.note ? <div style={{ fontSize: '15px', color: 'var(--text-secondary)', marginTop: '6px', whiteSpace: 'pre-wrap' }}>{doc.note}</div> : null}
      {doc.last_error ? (
        <div style={{ fontSize: '15px', color: 'var(--error)', marginTop: '6px', whiteSpace: 'pre-wrap', background: 'rgba(255, 68, 68, 0.08)', border: '1px solid rgba(255, 68, 68, 0.2)', borderRadius: '8px', padding: '8px 10px', lineHeight: 1.5 }}>
          {doc.last_error}
        </div>
      ) : null}
  ...
  const renderSourcesTab = () => (
    <div style={{ display: 'grid', gap: '10px' }}>
      ...
      {knowledgeActionNotice ? (
        <div style={{ fontSize: '15px', color: 'var(--success)', background: 'rgba(76, 175, 80, 0.08)', border: '1px solid rgba(76, 175, 80, 0.2)', borderRadius: '8px', padding: '8px 10px', marginBottom: '10px' }}>
          {knowledgeActionNotice}
        </div>
      ) : null}
      ...
      {knowledgeDocuments.value.length > 0 ? (
        ...
        <div style={{ ...panelStyle, fontSize: '15px', color: 'var(--text-muted)' }}>{t('knowledge_empty')}</div>
      )}
    </div>
  );
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', marginBottom: '4px' }}>
                  <div style={{ fontSize: '15px', color: 'var(--text-primary)', fontFamily: 'var(--font-ui)' }}>
                ...
                <div style={{ fontSize: '15px', color: 'var(--text-muted)', marginBottom: '4px', wordBreak: 'break-word' }}>{item.source || '—'}</div>
                <div style={{ fontSize: '15px', color: 'var(--text-secondary)', whiteSpace: 'pre-wrap' }}>{item.snippet || '—'}</div>
              </div>
            );
          })}
        </div>
      ) : (
        <div style={{ fontSize: '15px', color: 'var(--text-muted)' }}>{t('knowledge_search_empty')}</div>
      )}
    </div>
  );
      ...
      {activeDoc ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <div>
            ...
            <div style={{ fontSize: '15px', color: 'var(--text-primary)', fontFamily: 'var(--font-ui)', marginBottom: '4px' }}>
              {activeDoc.title || activeDoc.source}
            </div>
            <div style={{ fontSize: '15px', color: 'var(--text-muted)', marginBottom: '8px', wordBreak: 'break-word' }}>{activeDoc.source}</div>
            ...
            {activeDoc.last_hit_query ? (
              <div style={{ fontSize: '15px', color: 'var(--text-secondary)', marginBottom: '8px', whiteSpace: 'pre-wrap' }}>
                {`${t('knowledge_last_query')}: ${activeDoc.last_hit_query}`}
              </div>
            ) : null}
            {activeDoc.note ? (
              <div style={{ fontSize: '15px', color: 'var(--text-secondary)', marginBottom: '8px', whiteSpace: 'pre-wrap' }}>{activeDoc.note}</div>
            ) : null}
            {activeDoc.last_error ? (
              <div style={{ fontSize: '15px', color: 'var(--error)', marginBottom: '8px', whiteSpace: 'pre-wrap', background: 'rgba(255, 68, 68, 0.08)', border: '1px solid rgba(255, 68, 68, 0.2)', borderRadius: '8px', padding: '8px 10px', lineHeight: 1.5 }}>
                {activeDoc.last_error}
              </div>
            ) : null}
              ...
              <div key={chunk.chunk_id} style={{ border: '1px solid var(--border)', borderRadius: '6px', padding: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', marginBottom: '4px' }}>
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' }}>
                    <div style={{ fontSize: '15px', color: 'var(--text-primary)', fontFamily: 'var(--font-ui)' }}>{chunk.chunk_id}</div>
                    {chunk.page_label ? <span style={pillStyle()}>{chunk.page_label}</span> : null}
                  </div>
                  <span style={pillStyle()}>{formatTime(chunk.updated_at_ms)}</span>
                </div>
                <div style={{ fontSize: '15px', color: 'var(--text-secondary)', whiteSpace: 'pre-wrap' }}>{chunk.text || '—'}</div>
              </div>
            )) : <div style={{ fontSize: '15px', color: 'var(--text-muted)' }}>{t('knowledge_search_empty')}</div>,
          ...
          {renderAccordionSection(
            ...
            activeFacts.length > 0 ? activeFacts.map((fact) => (
              <div key={fact.fact_id} style={{ border: '1px solid var(--border)', borderRadius: '6px', padding: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', marginBottom: '4px' }}>
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' }}>
                    <div style={{ fontSize: '15px', color: 'var(--text-primary)', fontFamily: 'var(--font-ui)' }}>{fact.fact_id}</div>
                    {fact.page_label ? <span style={pillStyle()}>{fact.page_label}</span> : null}
                  </div>
                  <span style={pillStyle()}>{formatTime(fact.updated_at_ms)}</span>
                </div>
                <div style={{ fontSize: '15px', color: 'var(--text-secondary)', whiteSpace: 'pre-wrap' }}>{fact.claim || '—'}</div>
              </div>
            )) : <div style={{ fontSize: '15px', color: 'var(--text-muted)' }}>{t('knowledge_search_empty')}</div>,
        ...
        <div style={{ fontSize: '15px', color: 'var(--text-muted)' }}>{t('knowledge_detail_empty')}</div>
      )}
    </div>
  );
  ...
  const renderMemoryTab = () => {
    if (!snapshot) {
      return (
        <div style={panelStyle}>
          <div style={{ fontSize: '15px', color: 'var(--text-muted)' }}>{t('memory_loading')}</div>
        </div>
      );
    }
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        <div style={{ ...panelStyle, fontSize: '15px', color: 'var(--text-muted)', lineHeight: 1.6 }}>
        ...
        {creating ? (
          <div style={{ ...panelStyle, display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
              <button onClick={() => setCreating(false)} style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '15px' }}>
                {t('memory_cancel')}
              </button>
              <button onClick={() => void handleCreateEntity()} style={{ background: 'var(--purple)', border: 'none', borderRadius: '4px', color: '#fff', cursor: 'pointer', fontSize: '15px', padding: '4px 8px' }}>
                {t('memory_create')}
              </button>
            </div>
          </div>
            ...
            <div style={panelStyle}>
              ...
              {editingCore ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                    <button onClick={() => setEditingCore(false)} style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '15px' }}>
                      {t('memory_cancel')}
                    </button>
                    <button onClick={() => void handleSaveCore()} style={{ background: 'var(--purple)', border: 'none', borderRadius: '4px', color: '#fff', cursor: 'pointer', fontSize: '15px', padding: '4px 8px' }}>
                      {t('memory_save')}
                    </button>
                  </div>
                </div>
              ) : (
                <div style={{ fontSize: '15px', color: 'var(--text-primary)', whiteSpace: 'pre-wrap' }} onDblClick={() => { setEditingCore(true); setCoreDraft(snapshot.core || ''); }}>
                  {snapshot.core}
                </div>
              )}
            </div>
          ...
          {renderAccordionSection(
            ...
            filteredEntities.length > 0 ? filteredEntities.map((entity: MemoryEntityRecord) => (
              <div key={entity.name} style={{ border: '1px solid var(--border)', borderRadius: '6px', padding: '10px' }}>
                <div style={{ fontSize: '15px', fontFamily: 'var(--font-ui)', color: 'var(--text-primary)', marginBottom: '8px', display: 'flex', justifyContent: 'space-between' }}>
                  <span>{entity.name}</span>
                  {editingEntity !== entity.name ? (
                    <button onClick={() => { setEditingEntity(entity.name); setEditBody(entity.body || ''); }} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '15px' }}>
                      {t('memory_edit')}
                    </button>
                  ) : null}
                </div>
                ...
                {editingEntity === entity.name ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px' }}>
                      <button onClick={() => setEditingEntity(null)} style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '15px' }}>{t('memory_cancel')}</button>
                      <button onClick={() => void handleSaveEntity(entity.name)} style={{ background: 'var(--purple)', border: 'none', borderRadius: '4px', color: '#fff', cursor: 'pointer', fontSize: '15px', padding: '4px 8px' }}>{t('memory_save')}</button>
                    </div>
                  </div>
                ) : (
                  <div style={{ fontSize: '15px', color: 'var(--text-muted)', whiteSpace: 'pre-wrap' }} onDblClick={() => { setEditingEntity(entity.name); setEditBody(entity.body || ''); }}>
                    {entity.body}
                  </div>
                )}
              </div>
            )) : <div style={{ fontSize: '15px', color: 'var(--text-muted)' }}>{t('no_memory')}</div>,
          ...
          {renderAccordionSection(
            ...
            filteredRules.length > 0 ? filteredRules.map((rule, idx) => (
              <div key={`${rule.trigger || 'rule'}-${idx}`} style={{ border: '1px solid var(--border)', borderRadius: '6px', padding: '8px' }}>
                <div style={{ fontFamily: 'var(--font-ui)', fontSize: '15px', color: 'var(--accent)', marginBottom: '4px' }}>{rule.trigger || '—'}</div>
                <div style={{ fontSize: '15px', color: 'var(--text-primary)', marginBottom: '4px' }}>{rule.lesson || '—'}</div>
                <div style={{ fontSize: '15px', color: 'var(--text-muted)' }}>{rule.action || '—'}</div>
              </div>
            )) : <div style={{ fontSize: '15px', color: 'var(--text-muted)' }}>{t('memory_empty_rules')}</div>,
        ...
        {!snapshot.entities?.length && !snapshot.core && !snapshot.rules?.length ? (
          <div style={{ ...panelStyle, fontSize: '15px', color: 'var(--text-muted)' }}>{t('no_memory')}</div>
        ) : null}
      </div>
    );
  };
    ...
    <div style={{ ...panelStyle, padding: '12px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', marginBottom: '10px' }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ fontFamily: 'var(--font-ui)', fontSize: '15px', color: 'var(--purple)', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '8px' }}>
            // {t('memory_title')}
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
      ...
      {panelExpanded ? (
        <>
          ...
          {saveError || memoryError.value || knowledgeError.value ? (
            <div style={{ fontSize: '15px', color: 'var(--error)', fontFamily: 'var(--font-ui)', marginBottom: '8px', padding: '6px 8px', background: 'rgba(255,68,68,0.1)', borderRadius: '4px' }}>
              {saveError || memoryError.value || knowledgeError.value}
            </div>
          ) : null}
          {renderActiveTab()}
