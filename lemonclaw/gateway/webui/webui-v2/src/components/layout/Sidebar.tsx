...
export function Sidebar() {
  ...
  return (
    <aside class={`layout-sidebar ${mobileMenuOpen.value ? 'open' : ''}`}>
      <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, gap: '8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
          <img src="/logo-icon.svg" alt="" style={{ width: '22px', height: '22px', flexShrink: 0 }} />
          <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 600, fontSize: '16px', letterSpacing: '-0.5px', minWidth: 0 }}>
            Lemon<span style={{ color: 'var(--purple)', textShadow: '0 0 8px var(--purple-dim)' }}>Claw</span>
          </div>
        </div>
        <button class="sidebar-mobile-close" onClick={() => { mobileMenuOpen.value = false; }} style={{ background: 'transparent', border: 'none', color: 'var(--text-secondary)', fontSize: '22px', cursor: 'pointer', lineHeight: 1 }}>
          ×
        </button>
      </div>
