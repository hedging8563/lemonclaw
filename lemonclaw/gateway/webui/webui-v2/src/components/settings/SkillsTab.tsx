  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'inline-flex' }}>
      <button
        ...
        style={{ background: 'var(--bg-tertiary)', border: '1px solid var(--border)', borderRadius: '50%', width: '24px', height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: '15px', fontWeight: 'bold', cursor: 'pointer', fontFamily: 'var(--font-ui)', flexShrink: 0 }}
      >?</button>
      {show && (
        <div id={tooltipId} role="dialog" aria-label={t('supported_formats')} style={{ position: 'absolute', top: '32px', right: 0, zIndex: 50, background: 'var(--bg-tertiary)', border: '1px solid var(--border)', borderRadius: '6px', padding: '10px 12px', width: isMobile ? 'min(300px, calc(100vw - 48px))' : '300px', maxWidth: 'calc(100vw - 48px)', boxShadow: '0 4px 12px rgba(0,0,0,0.5)' }}>
          <div style={{ fontFamily: 'var(--font-ui)', fontSize: '15px', color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <div style={{ color: 'var(--text-primary)', fontSize: '15px', fontWeight: 'bold', marginBottom: '2px' }}>{t('supported_formats')}</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div style={{ background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: '8px', padding: isMobile ? '12px' : '14px', display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'stretch' : 'center', gap: '12px' }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontFamily: 'var(--font-ui)', fontSize: '15px', color: 'var(--text-primary)', marginBottom: '4px' }}>{t('skills_discovery_title')}</div>
          <div style={{ fontSize: '15px', color: 'var(--text-muted)', lineHeight: 1.5 }}>{t('skills_discovery_note')}</div>
        </div>
      ...
      <div style={{ display: 'flex', gap: '8px', alignItems: 'stretch', minHeight: '36px', flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
        ...
        <input
          ...
          style={{ flex: '1 1 260px', minWidth: 0, background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: '6px', padding: '0 16px', color: 'var(--text-primary)', fontFamily: 'var(--font-ui)', fontSize: '15px', outline: 'none', transition: 'border-color 0.2s', height: '36px', boxSizing: 'border-box' }}
      ...
      {installError && (
        <div role="alert" style={{ fontSize: '15px', color: 'var(--error)', fontFamily: 'var(--font-ui)', padding: '10px 12px', background: 'rgba(255, 68, 68, 0.08)', border: '1px solid rgba(255, 68, 68, 0.24)', borderRadius: '6px' }}>
          {installError}
        </div>
      )}
        ...
        {skills.map((skill) => (
          <div key={skill.name} style={{ background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: '6px', padding: '12px', display: 'flex', justifyContent: 'space-between', alignItems: isMobile ? 'stretch' : 'center', flexDirection: isMobile ? 'column' : 'row', gap: '12px' }}>
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ fontFamily: 'var(--font-ui)', fontSize: '15px', color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
              ...
              <div style={{ fontSize: '15px', color: 'var(--text-secondary)', marginTop: '4px', lineHeight: 1.5 }}>{skill.description}</div>
            </div>
            <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexShrink: 0, justifyContent: isMobile ? 'space-between' : 'flex-end' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '15px', color: skill.enabled ? 'var(--teal)' : 'var(--text-muted)', cursor: 'pointer', fontFamily: 'var(--font-ui)', whiteSpace: 'nowrap', userSelect: 'none', transition: 'color 0.2s' }}>
              ...
              {skill.source !== 'builtin' && (
                <button
                  ...
                  style={{ background: 'transparent', border: '1px solid transparent', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '15px', width: '28px', height: '28px', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}
