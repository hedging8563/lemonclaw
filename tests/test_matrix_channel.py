...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("lemonclaw.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "lemonclaw.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("lemonclaw.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "lemonclaw.channels.matrix.asyncio.create_task", _fake_create_task
    )
