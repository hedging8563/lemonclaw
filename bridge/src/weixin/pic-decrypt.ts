...
const DEFAULT_CDN_DOWNLOAD_TIMEOUT_MS = 10_000;

async function fetchCdnBytes(
  url: string,
  timeoutMs = DEFAULT_CDN_DOWNLOAD_TIMEOUT_MS,
  ...
  try {
    ...
    if (controller.signal.aborted) {
      throw new Error(`CDN download timed out after ${boundedTimeoutMs}ms`);
    }
    throw error;
  } finally {
...
export async function downloadAndDecryptBuffer(
  ...
  const encrypted = await fetchCdnBytes(url, timeoutMs);
  return {
...
export async function downloadPlainCdnBuffer(
  ...
  const url = buildCdnDownloadUrl(encryptedQueryParam, cdnBaseUrl);
  return fetchCdnBytes(url, timeoutMs);
}
