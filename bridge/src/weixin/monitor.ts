...
function eventFromMessage(accountId: string, message: WeixinMessage): WeixinBridgeEvent | null {
...
export class WeixinMonitorHub {
  ...
  private async monitorAccountLoop(accountId: string, controller: AbortController): Promise<void> {
    ...
    while (!controller.signal.aborted) {
      ...
      try {
        ...
        backoffMs = 2_000;
        this.handleGetUpdatesResponse(account, response);
      } catch (error) {
  ...
  private handleGetUpdatesResponse(account: WeixinAccountRecord, response: GetUpdatesResp): void {
    ...
    for (const message of messages) {
      const event = eventFromMessage(accountId, message);
      if (!event) continue;
